...
<head>
    ...
    <title>BB | Редактировать сотрудника</title>
...
<body class="antialiased">
    ...
    <section class="hero-section w-full relative flex items-center justify-center h-[200px] md:h-[300px]">
        ...
        <div class="relative z-20 text-center">
            <h1 class="text-3xl md:text-4xl font-bold text-[#8b5f4d] mb-4">Редактировать сотрудника</h1>
        </div>
    </section>
    ...
    <main>
        <div class="form-container">
            ...
            <form action="{{ route('admin.staff.update', $staff) }}" method="POST" enctype="multipart/form-data" onsubmit="return validateForm()">
                @csrf
                @method('PUT')
                
                <div>
                    <label class="form-label" for="first_name">Имя</label>
                    <input type="text" id="first_name" name="first_name" value="{{ old('first_name', $staff->first_name) }}" maxlength="50" class="form-input">
                    @error('first_name')<p class="error-text">{{ $message }}</p>@enderror
                </div>
                
                <div>
                    <label class="form-label" for="last_name">Фамилия</label>
                    <input type="text" id="last_name" name="last_name" value="{{ old('last_name', $staff->last_name) }}" maxlength="50" class="form-input">
                    @error('last_name')<p class="error-text">{{ $message }}</p>@enderror
                </div>
                
                <div>
                    <label class="form-label" for="position">Должность</label>
                    <input type="text" id="position" name="position" value="{{ old('position', $staff->position) }}" maxlength="30" class="form-input">
